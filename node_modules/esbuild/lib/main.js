...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.8.38") {
        throw new Error(`Cannot start service: Host version "${"0.8.38"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
              ...
              if (!rebuild) {
                ...
                rebuild.dispose = () => {
                  if (isDisposed)
                    return;
                  isDisposed = true;
                  if (pluginCleanup && --pluginCleanupCountdown === 0)
                    pluginCleanup();
            ...
            if (response.watchID !== void 0) {
              if (!stop) {
                ...
                stop = () => {
                  ...
                  if (pluginCleanup && --pluginCleanupCountdown === 0)
                    pluginCleanup();
                  sendRequest({command: "watch-stop", watchID: response.watchID}, () => {
                  });
                };
...
var startService = referenceCountedService(() => process.cwd(), (options) => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.8.38"}`), {
    cwd: process.cwd(),
    windowsHide: true,
  ...
  return Promise.resolve({
    build: (options2) => {
      return refPromise(new Promise((resolve, reject) => {
        let rebuild;
        let stop;
        let watch = options2.watch;
        if (watch && typeof watch !== "boolean") {
          watch = __assign({}, watch);
          ...
          if (typeof onRebuild === "function") {
            ...
            options2 = __assign(__assign({}, options2), {watch});
          }
        }
        service.buildOrServe("build", null, options2, isTTY(), (err, res) => {
          ...
          if (oldStop) {
            if (!stop) {
              refPromise(new Promise((resolve2) => {
                stop = () => {
                  resolve2();
                  oldStop();
                };
...
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.8.38"}`), {
    cwd: process.cwd(),
    windowsHide: true,
    input: stdin,
